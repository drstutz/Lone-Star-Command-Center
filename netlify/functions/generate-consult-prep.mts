...
CRITICAL — NEVER include range-of-motion (ROM) assessment anywhere in your output — not in examPriority, not in functionalTests. This means no test that measures or records DEGREES of rotation, flexion, extension, or lateral flexion in ANY body region, no matter how it's phrased. Banned examples include (but are not limited to): "Cervical ROM", "Lumbar ROM and forward flexion", "Cervical active range of motion", "Cervical rotation to full available range: R ___° / L ___°", "Lumbar flexion/extension ___°", or any variant asking to record a rotation/flexion/extension/lateral-flexion measurement in degrees. ROM is already fully captured separately via the SHS (Spinal Health Score) and re-measured at re-exam, so recommending any degree-based motion measurement here — under any name — is redundant. Focus examPriority entirely on orthopedic/neurological provocation tests, palpation, and special tests. Focus functionalTests on things that are NOT degree measurements: strength, endurance, balance, pain-with-movement, or task-specific function tied to the patient's actual goals.

Rules for functionalTests: pick 3-5 functional benchmark tests to re-measure at re-exam, chosen ONLY from tests that are actually relevant to this patient's complaint region and differential — do not default to a generic head-to-toe battery. For example, a patient with an isolated cervical/upper-body complaint and no lower-body involvement should NOT get gait/balance/leg tests like heel walk, toe walk, or single-leg balance; a patient with lower back/lower extremity involvement should. Good options include (choose only what's relevant): grip strength, heel walk, toe walk, squat depth/pain, one-leg balance, or a relevant provocation/functional movement tied to their stated goals (e.g. a golf swing test, a specific lifting motion). Do NOT include generic ROM degree measurements here (no "Cx Flexion ___°" style items) — that's covered by the SHS. Where a test naturally has a measurable/recordable outcome, bake a short blank into the test name itself for the doctor to fill in by hand, in this style: "Grip strength: R ___ / L ___", "Squat depth ___ pain ___/10", "One-leg balance: R ___ s / L ___ s", "Heel walk: Normal / Compensated". For tests that are simply pass/fail or observational, a plain test name with no blank is fine.`;
